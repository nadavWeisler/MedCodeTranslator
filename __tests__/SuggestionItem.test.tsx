...
describe('SuggestionItem', () => {
  it('renders the code', () => {
    const { getByText } = render(
      <SuggestionItem item={MOCK_ENTRY} lang="en" onPress={jest.fn()} schemeColor="#2563eb" t={t} />
    );
  ...
  it('renders Hebrew name when lang=he and name_he is available', () => {
    const { getByText } = render(
      <SuggestionItem item={MOCK_ENTRY} lang="he" onPress={jest.fn()} schemeColor="#2563eb" t={t} />
    );
  ...
  it('falls back to English name when lang=he but name_he is null', () => {
    const { getByText } = render(
      <SuggestionItem item={MOCK_ENTRY_NO_HE} lang="he" onPress={jest.fn()} schemeColor="#2563eb" t={t} />
    );
  ...
  it('calls onPress with the item when tapped', () => {
    ...
    const { getByText } = render(
      <SuggestionItem item={MOCK_ENTRY} lang="en" onPress={onPress} schemeColor="#2563eb" t={t} />
    );
  ...
  it('does not throw when schemeColor has alpha suffix', () => {
    expect(() =>
      render(<SuggestionItem item={MOCK_ENTRY} lang="en" onPress={jest.fn()} schemeColor="#2563eb" t={t} />)
    ).not.toThrow();
  });
});
